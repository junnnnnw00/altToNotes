...
**핵심 개념**
이 슬라이드는 자바의 패턴 매칭에서 발생할 수 있는 문제점과 `sealed` 클래스/인터페이스를 활용하여 이를 해결하는 방법을 설명합니다. 특히 `sealed` 키워드는 특정 인터페이스나 클래스를 상속받을 수 있는 자식 클래스들을 명시적으로 제한하여, `switch` 문과 같은 패턴 매칭에서 모든 가능한 경우의 수를 컴파일러가 인식하고 검증할 수 있도록 돕습니다. 이를 통해 `default` 구문 없이도 완전한(exhaustive) 패턴 매칭을 구현할 수 있게 됩니다.

**코드/수식 해설**

```java
record Pair<T>(T x, T y) {} // 제네릭 타입 T를 가지는 Pair 레코드 (불변 객체)

sealed interface I permits C, D {} // 인터페이스 I는 C와 D만 구현을 허용하는 sealed 인터페이스

record C(String s) implements I {} // I를 구현하는 레코드 C
record D(String s) implements I {} // I를 구현하는 레코드 D

static void exhaustiveSwitch(Pair<I> p) {
    switch (p) { // Pair<I> 타입 p에 대한 switch 표현식
        case Pair<I>(I i, C c) -> System.out.println("C = " + c.s()); // p의 두 번째 요소가 C 타입인 경우
        case Pair<I>(I i, D d) -> System.out.println("D = " + d.s()); // p의 두 번째 요소가 D 타입인 경우
    }
}
```

*   `record Pair<T>(T x, T y) {}`: `x`와 `y` 두 개의 필드를 갖는 `Pair` 레코드입니다. 레코드는 기본적으로 불변(immutable) 객체를 만드는데 최적화되어 있습니다.
*   `sealed interface I permits C, D {}`: `sealed` 키워드가 사용된 `I` 인터페이스는 오직 `permits` 절에 명시된 `C`와 `D` 레코드만이 이 인터페이스를 구현할 수 있도록 제한합니다. 이는 `I`의 모든 가능한 서브타입이 명확히 정의되어 있음을 컴파일러에게 알려줍니다.
*   `record C(String s) implements I {}` 및 `record D(String s) implements I {}`: `I` 인터페이스를 구현하는 두 개의 레코드입니다. 이들은 `I`가 허용하는 유일한 구현체입니다.
*   `exhaustiveSwitch(Pair<I> p)` 메서드 내의 `switch (p)`: `Pair<I>` 타입의 `p` 객체에 대해 패턴 매칭을 수행합니다.
    *   `case Pair<I>(I i, C c)`: `p`의 두 번째 요소가 `C` 타입일 때 이 패턴이 매칭됩니다. 여기서 `i`는 첫 번째 요소 `x`의 패턴 변수이고, `c`는 두 번째 요소 `y`의 패턴 변수입니다.
    *   `case Pair<I>(I i, D d)`: `p`의 두 번째 요소가 `D` 타입일 때 매칭됩니다.
*   이 `switch` 문은 `default` 구문이 없습니다. 이는 `I`가 `sealed` 인터페이스이고 `C`와 `D`만이 `I`를 구현할 수 있다는 정보 덕분에, `Pair<I>`의 두 번째 요소가 `C`이거나 `D`인 경우로 모든 가능성이 커버되기 때문에 컴파일러가 "완전한(exhaustive) 매칭"으로 간주하기 때문입니다.

**강의 맥락**
교수님은 `sealed` 인터페이스와 `permits` 키워드의 중요성을 강조하며, 일반적인 상속 관계에서 패턴 매칭을 사용할 때 발생할 수 있는 문제점(누군가 새로운 서브타입을 추가하면 기존 `switch` 문이 불완전해지고 컴파일러 에러 발생 가능성)을 언급했습니다. 이러한 문제를 해결하기 위해 `default` 키워드를 사용하여 예외를 처리하는 방법도 있지만, 때로는 모든 가능한 데이터 타입이 이미 알려져 있고 더 이상 확장을 원치 않을 때가 있다고 설명했습니다. 이럴 때 `sealed` 인터페이스를 사용하면, 인터페이스를 구현할 수 있는 클래스를 `permits` 키워드로 명시적으로 제한함으로써 `switch` 문에서 `default` 없이도 모든 경우의 수를 안전하게 처리할 수 있다고 강조했습니다. 즉, `sealed`를 사용하면 컴파일러가 모든 경우가 처리되었음을 확신하므로, 개발자는 불필요한 `default` 구문 없이 깔끔하고 안전한 코드를 작성할 수 있다는 점을 역설했습니다.

**시험 포인트**
*   ⭐ **`sealed` 인터페이스/클래스의 목적**: 왜 `sealed`를 사용하는가? (상속 계층 제한, 패턴 매칭의 완전성 보장)
*   ⭐ **`permits` 키워드의 역할**: `sealed` 키워드와 함께 사용되어 어떤 기능을 하는가?
*   ⭐ **완전한(Exhaustive) 패턴 매칭**: `sealed`를 통해 `default` 구문 없이 `switch` 문에서 모든 경우를 커버하는 방법과 그 이점.
*   ⭐ `record`, `sealed interface`, `implements`를 활용한 코드 예시에서 각 키워드의 의미와 동작 방식.
